...
func _load_available_units() -> void:
	...
	available_units = json_loader.load_json_file(AVAILABLE_UNITS_PATH)
	
	if available_units.is_empty():
		push_warning("[TeamManager] Aucune unité disponible chargée")
...
func add_to_team(unit_data: Dictionary) -> bool:
	...
	if current_team.size() >= 4:
		EventBus.notify("Équipe complète (max 4 en combat)", "warning")
		return false
...
func recruit_unit(unit_id: String) -> bool:
	...
	if roster.size() >= MAX_TEAM_SIZE:
		EventBus.notify("Roster complet (max " + str(MAX_TEAM_SIZE) + ")", "warning")
		return false
	
	# Vérifier si déjà recrutée
	for unit in roster:
		if unit.get("id") == unit_id:
			EventBus.notify("Unité déjà recrutée", "warning")
			return false
	...
	EventBus.notify("Recruté : " + new_unit.get("name"), "success")
	return true
...
func _level_up(unit: Dictionary) -> void:
	...
	EventBus.notify(unit.get("name") + " atteint le niveau " + str(unit.level) + " !", "success")
