...
func execute_last_man_stand(unit: BattleUnit3D) -> void:
	...
	if not check.can_use:
		...
		GlobalLogger.warning("LAST_MAN_STAND", check.reason)
		return
	
	GlobalLogger.info("LAST_MAN_STAND", "🔥 %s déclenche le %s !" % [unit.unit_name, LORE_NAME])
	...
	GlobalLogger.info("LAST_MAN_STAND", "Multiplicateur: %.2fx | Dégâts totaux: %d | Par case: %.1f" % [
		damage_multiplier,
		total_damage,
		damage_per_cell
	])
	...
	for pos in affected_positions:
		...
		if target and target.is_alive() and target.is_player_unit != unit.is_player_unit:
			...
			GlobalLogger.debug("LAST_MAN_STAND", "→ %s touché : %d dégâts" % [target.unit_name, damage])
	...
	GlobalLogger.info("LAST_MAN_STAND", "✅ %d ennemi(s) touché(s) | Total: %d dégâts" % [
		enemies_hit,
		total_damage_dealt
	])
	...
	# ✅ Message au joueur
	EventBus.notify("💀 %s : %s ! (%d dégâts)" % [unit.unit_name, LORE_NAME, total_damage_dealt], "critical")
