...
func _ready() -> void:
	GlobalLogger.info("ITEM_MODULE", "Module d'objets initialisé (%d objets)" % ITEM_DATABASE.size())

func setup_inventory(items: Dictionary) -> void:
	"""Configure l'inventaire pour le combat"""
	battle_inventory = items.duplicate()
	GlobalLogger.debug("ITEM_MODULE", "Inventaire: %s" % battle_inventory)
...
func use_item(user: BattleUnit3D, item_id: String, target: Variant = null) -> bool:
	...
	if success:
		...
		GlobalLogger.info("ITEM_MODULE", "%s utilise %s" % [user.unit_name, item_data.name])
		EventBus.notify("📦 %s utilise : %s" % [user.unit_name, item_data.name], "info")
...
func _apply_teleport(target: BattleUnit3D, user: BattleUnit3D, range: int) -> bool:
	...
	GlobalLogger.warning("ITEM_MODULE", "Téléportation non implémentée")
	return false
