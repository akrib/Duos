...
func _ready() -> void:
	GlobalLogger.info("REST_MODULE", "Module de repos initialisé (Max: %d points)" % MAX_REST_POINTS)
...
func reset_for_new_battle() -> void:
	...
	GlobalLogger.info("REST_MODULE", "Jauges de repos réinitialisées (%d points chacune)" % INITIAL_REST_POINTS)
...
func use_rest_point(unit: BattleUnit3D) -> bool:
	...
	if not can_use_rest(unit):
		GlobalLogger.warning("REST_MODULE", "%s : impossible d'utiliser le repos" % unit.unit_name)
		return false
	...
	GlobalLogger.info("REST_MODULE", "%s utilise 1 repos (reste: %d)" % [
		unit.unit_name,
...
func add_rest_point(is_player: bool, amount: int = 1) -> void:
	...
	GlobalLogger.info("REST_MODULE", "Faction %s : +%d repos (total: %d)" % [
