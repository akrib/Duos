...
func _ready() -> void:
	...
	GlobalLogger.debug("BATTLE_UNIT", "Unité %s initialisée (ID: %s, Mana: %s)" % [unit_name, unit_id, mana_type])
...
func consume_mana(amount: int) -> bool:
	...
	if current_mana < amount:
		GlobalLogger.warning("BATTLE_UNIT", "%s : mana insuffisant (%d/%d)" % [unit_name, current_mana, amount])
		return false
	...
	GlobalLogger.info("BATTLE_UNIT", "%s consomme %d mana (reste: %d/%d)" % [unit_name, amount, current_mana, max_mana])
...
func restore_mana(amount: int) -> int:
	...
	if actual_restore > 0:
		GlobalLogger.info("BATTLE_UNIT", "%s restaure %d mana (%d/%d)" % [unit_name, actual_restore, current_mana, max_mana])
...
func initialize_unit(data: Dictionary) -> void:
	...
	GlobalLogger.debug("BATTLE_UNIT", "  → HP: %d/%d" % [current_hp, max_hp])
	GlobalLogger.debug("BATTLE_UNIT", "  → Mana: %d/%d (%s)" % [current_mana, max_mana, mana_type])
